...
import { IFabricService } from './fabricService';
...
const mockDrugStatuses: DrugStatus[] = [
  {
    sgtin: 'SGTIN001',
    isRecalled: false,
    reason: '',
 ...
 * Register a new drug
 */
const registerDrug = async (drugData: Omit<Drug, 'id'>): Promise<Drug> => {
  const newDrug: Drug = {
 ...
 */
const getDrugsByOwner = async (ownerId: string): Promise<Drug[]> => {
 ...
 * Get drug by ID
 */
const getDrugById = async (id: string): Promise<Drug | null> => {
 ...
 * Get drug by SGTIN
 */
const getDrugBySGTIN = async (sgtin: string): Promise<Drug | null> => {
 ...
 */
const createEvent = async (eventData: Omit<TrackingEvent, 'id'>): Promise<TrackingEvent> => {
 ...
 */
const getEventsByDrug = async (drugId: string): Promise<TrackingEvent[]> => {
 ...
 * Get all events
 */
const getAllEvents = async (): Promise<TrackingEvent[]> => {
  return mockEvents;
};
 ...
 */
const getRecentEvents = async (limit: number = 10): Promise<TrackingEvent[]> => {
  return mockEvents.slice(-limit);
};

/**
 * Transfer a drug to a new owner
 */
const transferDrug = async (drugId: string, fromId: string, toId: string, toName: string, toRole: string, location: string, details: Record<string, any>): Promise<boolean> => {
 ...
 * Receive a drug
 */
const receiveDrug = async (drugId: string, receiverId: string, receiverName: string, receiverRole: string, location: string, details: Record<string, any>): Promise<boolean> => {
  const drugIndex = mockDrugs.findIndex(drug => drug.id === drugId);
  if (drugIndex === -1) {
    return false;
  }
...
/**
 * Initiate a recall for a drug
 */
const initiateRecall = async (sgtin: string, reason: string, initiator: any): Promise<boolean> => {
...
/**
 * Check the recall status of a drug
 */
const checkRecallStatus = async (sgtin: string): Promise<any> => {
  const status = mockDrugStatuses.find(status => status.sgtin === sgtin);
  if (!status) {
...
/**
 * Get detailed drug data by SGTIN (for public tracking)
 */
const getDrugDetailsBySGTIN = async (sgtin: string): Promise<any> => {
  ...
  const events = await getEventsByDrug(drug.id);
  
  // Check if drug is recalled
  const recallStatus = await checkRecallStatus(sgtin);
...
// Update the exported object to include the new method
export const mockBlockchainService: IFabricService = {
  registerDrug,
  getAllDrugs,
  getDrugsByOwner,
  getDrugById,
